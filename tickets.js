...
var ticket_price = 12000;

requestWakeLock();
$('.select_amount .item.fixed').click(function(){
	amount = $(this).attr('amount');
	$('.payment_method .qty .value').text(amount);
  $('.payment_method .amount .value').text(amount*ticket_price/100);
...
$('.select_amount .item.other').click(function(){
  $('.select_amount').removeClass('visible');
  $('.select_other_amount').addClass('visible');
  $('#other_amount').focus();
...
$('.other_amount_submit').click(function(){
	...
	$('.payment_method .amount .value').text(amount);
	$('.select_other_amount').removeClass('visible');
	$('.payment_method').addClass('visible');
});

$('.payment_method .item.cash').click(function(){
	$('.confirm_cash .amount .value').text(amount*ticket_price/100);
	$('.payment_method').removeClass('visible');
	$('.confirm_cash').addClass('visible');
});
...
$('.confirm_cash .item.yes').click(async function(){
  ...
  try {
    ...
    if(result._id){
      $('.top_up .amount .value').text(amount*ticket_price/100);
      $('.confirm_cash').removeClass('visible');
      $('.top_up').addClass('visible');
      $('.screen.top_up').css('background','#4db74d');
    } else {
      $('.top_up').addClass('visible');
      $('.confirm_cash').removeClass('visible');
      $('.screen.top_up').css('background','red');
      $('.title').text("Failed to write database.");
    }
  } catch (err) {
    $('.top_up').addClass('visible');
    $('.screen.top_up').css('background','red');
...
$('.success').click(function(){
  window.location.replace("/modules/tickets/");
});

$('.payment_method .item.card').on('click', function () {
    const ISVamount = amount*ticket_price*5/100;
    const vivaUrl = "vivapayclient://pay/v1" +
      ...
      "&amount="+ amount*ticket_price +                     // Amount in cents (e.g., 10000 = 100.00 RON)
      "&sourceCode=2879 " +
      "&callback=https://app.tadaa.ro/modules/tickets/callback/" +
            ...
            if(!firstCharge) {
              try {
                ...
                if(result.response.errors){
                  ...
                  $('.success .amount .value').text(amount);
