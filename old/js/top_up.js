...
$('.payment_method .item.cash').click(function(){
	$('.confirm_cash .amount .value').text(amount/100);
...
$('.select_amount .item.check_balance_buton').click(function(){
  $('.select_amount').removeClass('visible');
  $('.check_balance').addClass('visible');
    ...
    (async () => {
      try {
        ...
        ndef.onreading = async (event) => { // make this async!
          ...
          if (error) {
            $('.check_balance .error').append(error);
          }

          if (!TDID) {
            $('.check_balance .error').append("No TDID found <br>");
          } else {
            //topup
            ...
            try {
              ...
              // console.log(response);
              if(result.balance){
                $('.check_balance .value .amount').text(result.balance/100);
                $('.check_balance .value').show();
              } else {
                $('.check_balance .error').append("Tag not found").show();
              }
            } catch (err) {
...
$('.success').click(function(){
  window.location.replace("/modules/top_up/");
});
...
$('.payment_method .item.card').on('click', function () {
  const ISVamount = amount*5/100;
    const vivaUrl = "vivapayclient://pay/v1" +
      ...
      "&sourceCode=4677" +
      "&callback=https://app.tadaa.ro/modules/top_up/callback/" +
      "&ISV_amount=0" +
