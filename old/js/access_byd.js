if($('#scanInput').length){
    document.getElementById('scanInput').addEventListener('keydown', function (e) {
        ...
        if (e.key === 'Enter' || e.keyCode === 13) {
            ...
            $.post('/php/check_ticket_byd.php', { id: scannedValue}, function(response) {
                //$('#ticketCode').text(JSON.stringify(response, null, 2));

              

                if(response.valid && response.istoday) {
                    $('#ticketCode').html('<br>Bilet valid<br>'+response.ticket_name+'<br>'+response.ticket_category);
                    ...
                    $('#ticketCode').html('<big>Codul a mai fost folosit</big><br><br>'+response.ticket_name+'<br>'+response.ticket_category);
