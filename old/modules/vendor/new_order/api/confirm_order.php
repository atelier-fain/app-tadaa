...
if (strcasecmp($_SERVER['REQUEST_METHOD'], 'post') == 0)
{
    if(isset($_POST['env_key']) && isset($_POST['data']))
    {
        ...
        $privateKeyFilePath = 'Mobilpay/security/sandbox/sandbox.2HGG-OEED-CVCY-L8XR-CKHMprivate.key';
        //$privateKeyFilePath = 'Mobilpay/security/live/live.2HGG-OEED-CVCY-L8XR-CKHMprivate.key';
        ...
        //print_r($objPmReq);
        $netopia_obj = (array)$objPmReq;


        file_put_contents("order_confirm.log", "---------".$objPmReq->objPmNotify->errorCode."--------",FILE_APPEND);
        file_put_contents("order_confirm.log", "---------".$objPmReq->objPmNotify->errorMessage."--------",FILE_APPEND);
        file_put_contents("order_confirm.log", "---------".$netopia_obj['params']['vendor']."--------",FILE_APPEND);
        file_put_contents("order_confirm.log", "---------".$netopia_obj['params']['_id']."--------",FILE_APPEND);
        file_put_contents("order_confirm.log", json_encode($netopia_obj),FILE_APPEND);
        //file_put_contents("order_confirm.log", print_r($netopia_obj),FILE_APPEND);
        ...
        if ($objPmReq->objPmNotify->errorCode == 0) {
            ...
            // file_put_contents("order_confirm.log", "---------2--------",FILE_APPEND);
        }
        else {
...
if ($objPmReq->objPmNotify->errorCode == 0) {
  order_mark_as_paid($netopia_obj['params']['_id'],$netopia_obj['params']['vendor'],$objPmReq->orderId,$objPmReq->objPmNotify->errorCode,$objPmReq->objPmNotify->errorMessage);
} else {
  order_mark_as_unpaid($netopia_obj['params']['_id'],$objPmReq->orderId,$objPmReq->objPmNotify->errorCode,$objPmReq->objPmNotify->errorMessage);
}
